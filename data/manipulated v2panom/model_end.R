...
#this is where Darin's team can recode - move 1 to 4 (outside of party control)
wdata.df <- transform(wdata.df,
  ...
  mutate(code = case_match(code, 1 ~ 5, 2 ~ 1, 3 ~ 2, 4 ~ 3, 5 ~ 4))
...
posterior.sim <- stan(file = file.path("/Users/selfd/elements/vparty", stanfile),
                      data = data,
                      iter = ITER,
                      thin = THIN,
                      chains = CHAINS,
