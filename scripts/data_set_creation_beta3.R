...
#join with party age/dissolution data
age <- read_csv("data/vparty_age.csv") %>%
  rename(year = year...20, country_name = country_name...16, party_id = pf_party_id)

party_inst3 <- left_join(party_inst2, age, by = c("year", "country_id", "party_id")) %>%
  rename(country_name = country_name.x, COWcode = COWcode.x, v2pasoctie = v2pasoctie.x, v2paactcom = v2paactcom.x, v2pasoctie = v2pasoctie.x, v2palocoff = v2palocoff.x,
         v2panom = v2panom.x, v2paelcont = v2paelcont.x) %>%
  select(-c("year...2", "country_name...4", "...14", "country_name.y", "COWcode.y"))

#### output the names  
write.csv(party_inst3, file = "data/pips_beta3.csv", row.names = FALSE)
